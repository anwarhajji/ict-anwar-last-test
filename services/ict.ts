...
export const detectEntries = (
    ...
    // TRACKING FOR ONE TRADE PER DAY (8 AM Hour)
    const daily8AMTaken = new Set<string>();

    for (let i = 100; i < data.length; i++) {
        const candle = data[i];
        const candleDate = new Date((candle.time as number) * 1000);
        const hour = candleDate.getUTCHours();
        const dateKey = candleDate.toDateString(); // "Mon Jan 01 2024"
        ...
        if (touchingBullOB || touchingBullFVG) {
            if (inDiscount || isScalping) {
                ...
                if (direction) {
                    ...
                    // 8 AM Hour Strategy
                    const is8AM = (hour === 12 || hour === 13); // UTC (8-9 AM EST/EDT)
                    if (is8AM && foundSweep && (touchingBullOB || touchingBullFVG)) {
                        // Check One Per Day Constraint
                        if (daily8AMTaken.has(dateKey)) {
                            // Already took trade today
                            direction = null; 
                        } else {
                            setupName = '8 AM Hour';
                            score += 5;
                            confluences.push('NY Open Sweep');
                            daily8AMTaken.add(dateKey); // Mark today as taken
                        }
                    }
        ...
        // --- SHORT ---
            ...
            if (inPremium || isScalping) {
                ...
                if (direction) {
                    ...
                    // 8 AM Hour Strategy
                    const is8AM = (hour === 12 || hour === 13); // UTC (8-9 AM EST/EDT)
                    if (is8AM && foundSweep && (touchingBearOB || touchingBearFVG)) {
                        // Check One Per Day Constraint
                        if (daily8AMTaken.has(dateKey)) {
                            // Already took trade today
                            direction = null; 
