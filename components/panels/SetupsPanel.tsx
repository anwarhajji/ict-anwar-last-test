...
export const MODEL_INFO: Record<string, { desc: string; rules: string[] }> = {
    ...
    '8 AM Hour': {
        desc: "A New York Session Open reversal strategy targeting liquidity sweeps followed by key level reactions. STRICT: One trade per day max.",
        rules: [
            "Time Window: 8:00 AM - 9:00 AM New York time (12:00-13:00 UTC approx).",
            "Identify a sweep of a key Swing High or Swing Low.",
            "Price must react off a PD Array (OB or FVG) immediately after sweep.",
            "Target the opposing liquidity level.",
            "Limit: Only 1 trade execution allowed per day for this model."
        ]
    }
};
...
export const SetupsPanel: React.FC<SetupsPanelProps> = ({ entries, overlays, setOverlays, setClickedEntry, onClose, onViewOnChart }) => {
    ...
    const stats = useMemo(() => {
        ...
        return types.map(type => {
            ...
            const history = relevant.sort((a,b) => (b.time as number) - (a.time as number)).slice(0, 30);
            
            return { type, total, wins, losses, winRate, pnl, history, fullHistory: relevant };
        });
    }, [entries]);
    ...
    // 15-Day Journal Logic for 8 AM Hour
    const dailyJournal = useMemo(() => {
        if (selectedModel !== '8 AM Hour' || !activeModelStats) return [];
        ...
        const historyMap = new Map<string, EntrySignal>();
        activeModelStats.fullHistory.forEach(trade => {
            const dateStr = new Date((trade.time as number) * 1000).toDateString();
            // Since logic enforces 1 per day, we just take the one present
            historyMap.set(dateStr, trade);
        });
        ...
        for (let i = 0; i < 15; i++) {
            ...
            const trade = historyMap.get(dateStr);
            journal.push({ date: d, trade });
        }
        return journal;
    }, [selectedModel, activeModelStats]);
    ...
    const is8AMHour = selectedModel === '8 AM Hour';
            ...
            <div className="max-w-7xl mx-auto">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
                    ...
                    <div className="lg:col-span-2 space-y-6">
                        ...
                        {/* Trade History Table or Specialized Journal */}
                        {is8AMHour ? (
                             <div className="bg-[#151924] border border-[#2a2e39] rounded-xl overflow-hidden shadow-lg animate-in fade-in">
                                <div className="p-4 border-b border-[#2a2e39] bg-[#1e222d] flex justify-between items-center">
                                    <h3 className="font-bold text-white">15-Day Performance Journal</h3>
                                    <span className="text-xs text-yellow-500 font-bold border border-yellow-500/50 px-2 py-0.5 rounded">ONE TRADE PER DAY</span>
                                </div>
                                <div className="p-4 grid grid-cols-1 gap-3">
                                    {dailyJournal.map((day, i) => {
                                        const hasTrade = !!day.trade;
                                        const isWin = day.trade?.backtestResult === 'WIN';
                                        
                                        return (
                                            <div key={i} className="flex items-center justify-between p-3 bg-[#0b0e11] rounded border border-[#2a2e39] hover:border-blue-500/50 transition-colors">
                                                <div className="flex items-center gap-4">
                                                    <div className="text-gray-500 font-mono text-xs w-24">{day.date.toLocaleDateString()}</div>
                                                    <div className="w-px h-6 bg-[#2a2e39]"></div>
                                                    {hasTrade ? (
                                                        <div className="flex items-center gap-3">
                                                             <span className={`text-xs font-bold px-2 py-0.5 rounded ${day.trade?.type === 'LONG' ? 'bg-green-900/20 text-green-400' : 'bg-red-900/20 text-red-400'}`}>
                                                                {day.trade?.type}
                                                            </span>
                                                            <span className={`text-xs font-bold ${isWin ? 'text-green-500' : 'text-red-500'}`}>
                                                                {day.trade?.backtestResult}
                                                            </span>
                                                            <span className="text-xs text-gray-500 font-mono">
                                                                (@ {day.trade?.price.toFixed(2)})
                                                            </span>
                                                        </div>
                                                    ) : (
                                                        <span className="text-xs text-gray-600 italic">No valid setup detected</span>
                                                    )}
                                                </div>
                                                ...
                                                <div className="flex items-center gap-4">
                                                    {hasTrade ? (
                                                        <>
                                                            <div className={`font-mono font-bold text-sm ${day.trade!.backtestPnL! >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                                                                {day.trade!.backtestPnL! > 0 ? '+' : ''}{day.trade!.backtestPnL!.toFixed(2)}
                                                            </div>
                                                            <button 
                                                                onClick={() => onViewOnChart(day.trade!)}
                                                                className="text-blue-400 hover:text-white text-xs bg-blue-900/20 px-2 py-1 rounded border border-blue-900/50 hover:bg-blue-600"
                                                            >
                                                                Review
                                                            </button>
                                                        </>
                            ...
                            <div className="bg-[#151924] border border-[#2a2e39] rounded-xl overflow-hidden shadow-lg">
                                <div className="p-4 border-b border-[#2a2e39] bg-[#1e222d] flex justify-between items-center">
                                    <h3 className="font-bold text-white">Last {stat.history.length} Executions</h3>
